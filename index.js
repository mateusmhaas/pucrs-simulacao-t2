...
//Agenda inicial
var eventos = [{ key: "ENTRADA", tempo: 1, sorteio: 1 }];
...
const eventoSA = (evento, previous) => {
  ...
  if (previous.fila >= previous.C)
    agenda(
      random_next_event(evento.previous),
      evento.previous,
      evento["tempo"],
      get_random(previous.SA),
      "SA1"
    );
};
/**
...
const eventoPIPE = evento => {
  ...
  if (current) eventoCH(evento, current);

  if (previous) eventoSA(evento, previous);
};
...
const run = () => {
  for (var k in queues) {
    ...
    for (var i = 0; i <= queues[k].K; i++) queues[k].estados[i] = 0;
  }
...
const print_statistics = () => {
  ...
  for (var k in queues) {
    ...
    queues[k].estados
      ...
      .forEach((value, index) => {
        ...
        message2 += `${k},${index},${value.toFixed(4)}\n`;
      });
    ...
    probs
      ...
      .forEach((value, index) => {
        ...
        message3 += `${k},${index},${value.toFixed(4)}\n`;
      });
  ...
  console.log("FILA,PERDAS");
  console.log(message1);
  console.log("FILA,ESTADO,TEMPO");
  console.log(message2);
  console.log("FILA,ESTADO,PROBABILIDADE");
